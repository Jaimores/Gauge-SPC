...
library(sVMisc)  #Visualitation of progres´s fucntion
...
#Tail´s probability and parameters associated to (Normal, Lognormal2, Lognormal3, Weibull3, Skewnormal) distributions
# when gaugés dimensions are allocated in (q) ang there are a shift (delta,r) in the proccess. 
...
ARLwYsYl.EWMA<-function(n,Lim,w,lambda,q,delta,r,m,opt=1,mu=10,sig=1,dist="norm",skew=0,image=F){
  ...
  Mst=Mlt=Mt=Prob.IC=Prob.OOC=0;p.IC=p.OOC=0;     #se inician los vectores para construir la distribuciÃ³n
  ...
  if(opt==1){
    # construye la distribución de max(wYs+Yl, Ys+wYl)
    for (s in 0:n){
      fact.s<-factorial(s)
      for (l in 0:(n-s)){
        h=h+1
    ...
    #resume la distribuciÃ³n, eliminando valores repetidos
    ...
    #Calcula estadísticos resumen, ------ojo Min puede variar
    Min<-est[1]
    Var<-E2-(Mu^2)
    ...
    #Crea y llena Matrice de probabilidades de transición bajo control P, fuera de control Q.
    ...
    for(i in 1:m){                                    #Recorre todos los estados iniciales
      ...
      for(h1 in 1:length(est)){                       #Para todos los valores del estadistico (est), evalua el intervalo de llegada
        ...
        if(z<LC){
          j=1+floor(max((z-Min)/amp,0))               #Identifica el estado j al que llegará Zt
    ...
    # construye la distribución de wYs+Yl y de Ys+wYl
    for (s in 0:n){                                 
      fact.s<-factorial(s)
      for (l in 0:(n-s)){
        cte<-fact.n/(fact.s*factorial(l)*factorial(n-s-l))
...
ARLwYsYl.DS<-function(n1,n2,Lc1,Lw,Lc2,w,q,delta,r,mu=10,sig=1,dist="norm",skew=0){
  ...
  #Probabilidad de no señal en 2 es el complemento
...
#### Decode, fitness and GA Functión for D2YSYL GA with one shift or three shift
...
fitn.D2YsYl<-function(string,l,maxim,mobj){
   ...
   #fitn<-10000-w1*((ARL0<ARL0obj)*6*abs(ARL0-ARL0obj)+(ARL0>=ARL0obj)*abs(ARL0-ARL0obj))-w2*FObj1}
     fitn<-10000-w1*6*(ARL0<ARL0obj)*(ARL0obj-ARL0)-w2*FObj1}
     }
   else{
...
GA.D2YsYl<-function(n,delta,r,ARL0obj,mu,sig,dist,skew,mobj=T){
  ...
  l2<-length(decimal2binary((q.max-q.min)/prec.q))#q
  l<-c(l1,l2)
  maxim<-0
  for(i in 1:length(l)){
...
#### Decode, fitness and GA Functión for wYSYL TMV with one shift
...
decode.wYsYlTMV <- function(string,n2.max,maxim,l,F.Lc1,F.w,F.n1) {#l:longitud de las cadenas,
   ...
   n1 <- max(ceiling((n-1)*binary2decimal(string[(sum(l[1:2])+1):sum(l[1:3])])/maxim[3]),1)# max(1, n-1-binary2decimal(string[(sum(l[1:2])+1):sum(l[1:3])]))  #esto le da mas probabilidad a los numeros pequeños
   }
   n2 <- n + max(ceiling((n2.max-n)*binary2decimal(string[(sum(l[1:3])+1):sum(l[1:4])])/maxim[4]),1)#max(n+1,n2.max-binary2decimal(string[(sum(l[1:3])+1):sum(l[1:4])])) #esto le da mas probabilidad a los numeros pequeños
   Lw <- L.min + (1-L.min)*(binary2decimal(string[(sum(l[1:4])+1):sum(l[1:5])]))/maxim[5]    #max(LC.min, 1-binary2decimal(string[(sum(l[1:4])+1):sum(l[1:5])])*prec.LC)
   Lc.min<- Lw +0.05
...
fitn.wYsYlTMV<-function(string,n2.max,maxim,l,F.Lc1,F.w,F.n1,fit){
   ...
   if(fit==1){ fitn<-10000-w1*((ARL[1]<ARL0obj)*(ARL0obj-ARL[1]))-w3*(ARL[2]>n)*(ARL[2]-n)-w2*ARL[4]}
     else{fitn<-10000-w1*((ARL[1]<ARL0obj)*2*abs(ARL[1]-ARL0obj)+(ARL[1]>=ARL0obj)*abs(ARL[1]-ARL0obj))-w3*((ARL[2]<n)*(n-ARL[2])+2*(ARL[2]>=n)*(ARL[2]-n))-w2*ARL[4]}
    }
   fitn
}
...
GA.wYsYlTMV<-function(n,delta,r,ARL0obj,n2.max=2*n,mu,sig,dist,skew,maxiter,F.Lc1=NA,F.w=NA,F.n1=NA,fit=2){
  ...
  par<-c(n,delta,r,ARL0obj)
  names(par)<-c("n","delta","r","ARL.0 deseado")
  w.min<-(-1.1);w.max<-1.1; prec.w<-0.05
  q.min<-0.001;q.max<-0.65; prec.q<-0.0001
  L.min<-0.001; prec.L<-0.01 ###Fundamental para la precision del algoritmo.....(0.05, mejor opcion)
  
  if(delta==0){l1<-8}else{l1<-9}
  ...
  l5<-length(decimal2binary(1.05/prec.L+1))#Lw
  if(is.na(F.Lc1)==F){l6<-0}else{
  l6<-length(decimal2binary(1/prec.L+1))#LC1
  }
  l7<-length(decimal2binary(1/prec.L+1))#LC2
  ...
  GA.sol<-ga(type = "binary", nBits = sum(l), fitness = fitn.wYsYlTMV,l=l,maxim=maxim,F.Lc1=F.Lc1,F.w=F.w,n.max,F.n1,fit=fit,
             popSize = 500, maxiter = maxiter, run = 110, pcrossover = 0.95, pmutation= 0.25) #.pendiente modificar tipo d cruce y seleccion
...
#### Decode, fitness and GA Functión for wYSYL DS with one shift
...
decode.wYsYlDS <- function(string,n.max,maxim,l,F.Lc1,F.w,F.n1) {#l:longitud de las cadenas,
  ...
  n2 <- (n-n1)+max(ceiling((n.max-n)*binary2decimal(string[(sum(l[1:3])+1):sum(l[1:4])])/maxim[4]),1)#max(n+1,n2.max-binary2decimal(string[(sum(l[1:3])+1):sum(l[1:4])])) #esto le da mas probabilidad a los numeros pequeños
...
fitn.wYsYlDS<-function(string,n.max,maxim,l,F.Lc1,F.w,F.n1,fit){
  ...
  if(ARL[1]==Inf|ARL[4]==Inf|ARL[1]<1|ARL[4]<1){fitn<-0}
    ...
    if(fit==1){fitn<-10000-w1*max(ARL0obj-ARL[1],0)-w3*max(ARL[2]-n,0)-w2*ARL[4]}
    else{fitn<-10000-w1*((ARL[1]<ARL0obj)*2*abs(ARL[1]-ARL0obj)+(ARL[1]>=ARL0obj)*abs(ARL[1]-ARL0obj))-w3*((ARL[2]<n)*(n-ARL[2])+2*(ARL[2]>=n)*(ARL[2]-n))-w2*ARL[4]}
  }
  fitn
...
SARLwYsYl.EWMA<-function(n,Lim,w,lambda,q,delta,r,opt=1,mu=10,sig=1,dist="norm",skew=0,it=30000){#Simulador wYsYl
  ...
  #Encuentra los parametros de la distribuciÃ³n fuera de control del proceso
  if(dist=="norm"){
  ...
  ##ojoooooo, no esta incluid la distribuciÃ³n weibull
  ...
  }else{Mu=n*(q/2)*(w+1);Var=n*q/2*(1-q/2)*(w*w+1-w*q/(1-q/2))}    # Media y valor esperado para opciÃ³n 2.
  
  LC<-Mu+Lim*sqrt(Var*lambda/(2-lambda))      #Factor L se convierte en Limite de Control LC
    ...
    while (EC=="True"){
      ...
      if(dist=="norm"){Dat= rnorm(n,mu1,sig1)}else if(dist=="snorm"){
        Dat= rsn(n,xi1,omega1,lambda1)}else{
          Dat= rlnorm3(n,gamma1,mulog1,siglog1)}     ##ojoooooo, no esta incluid la distribuciÃ³n weibull
      ...
      if(opt==1){
        ...
        Zt=max(Zlt,Zst)}
      if(Zt<LC){EC="True"}else{EC="False"}
    }
    LR[j]=rl                                    # Almacena valor de la longitud de racha e inicia nueva iteraciÃ³n
  }
